...
def load_config():
    ...
    config = {
        ...
        "PUBLIC_REPORT_URL": config_data["app"].get("public_report_url", ""),
        "GITHUB_PAGES_DOCS_DIR": config_data["app"].get("github_pages_docs_dir", ""),
        "REQUEST_INTERVAL": config_data["crawler"]["request_interval"],
        "REPORT_MODE": os.environ.get("REPORT_MODE", "").strip()
        or config_data["report"]["mode"],
        "RANK_THRESHOLD": config_data["report"]["rank_threshold"],
        "MATCH_LOOSE_MODE": config_data["report"].get("match_loose_mode", False),
        ...
        "FEISHU_MESSAGE_TYPE": config_data["notification"].get(
            "feishu_message_type", "text"
        ),
        "COMBINE_CURRENT_DAILY": config_data["notification"].get(
            "combine_current_daily", False
        ),
        "COMBINE_DAILY_INCREMENTAL": config_data["notification"].get(
            "combine_daily_incremental", False
        ),
        "COMBINE_CURRENT_INCREMENTAL": config_data["notification"].get(
            "combine_current_incremental", False
        ),
        ...
        "FEISHU_ITEM_BULLET": config_data["notification"].get(
            "feishu_item_bullet", "•"
        ),
        "FEISHU_NEW_ICON": config_data["notification"].get(
            "feishu_new_icon", "🆕"
        ),
        "PUSH_WINDOW": {
...
def matches_word_groups(
    ...
    use_loose = CONFIG.get("MATCH_LOOSE_MODE", False)
    title_lower = title.lower() if not use_loose else normalize_text(title)

    # 过滤词检查
    if any((normalize_text(filter_word) if use_loose else filter_word.lower()) in title_lower for filter_word in filter_words):
        return False
    ...
    for group in word_groups:
        ...
        if required_words:
            all_required_present = all(
                (normalize_text(req_word) if use_loose else req_word.lower()) in title_lower
        ...
        if normal_words:
            any_normal_present = any(
                (normalize_text(normal_word) if use_loose else normal_word.lower()) in title_lower
...
def send_to_feishu(
    ...
    for i, batch_content in enumerate(batches, 1):
        ...
        def strip_html_for_feishu(text: str) -> str:
            import re
            try:
                text = re.sub(r"<a\s+href=['\"](.*?)['\"]\s*>(.*?)</a>", lambda m: f"{m.group(2)} ({m.group(1)})", text, flags=re.IGNORECASE)
            except Exception:
                pass
            text = re.sub(r"<[/]?[^>]+>", "", text)
            return text
        ...
        if CONFIG.get("FEISHU_MESSAGE_TYPE", "text") == "post":
            ...
            link = (base.rstrip("/") + "/reports/" + date_str + ".html") if base else ""

            paragraphs = []

            title_text = f"📊 热点词汇统计"
            paragraphs.append([{"tag": "text", "text": title_text}])
            ...
            if is_combined:
                ...
                for sec in sections:
                    icon = sec.get("icon", "")
                    title = sec.get("title", "")
                    paragraphs.append([{"tag": "text", "text": f"{icon} {title}"}])
                    stats_list = [s for s in sec.get("stats", []) if s.get("count", 0) > 0]
                    total_groups = len(stats_list)
                    # 对增量段显示“共 X 条”
                    if title == "本次新增热点新闻":
                        total_count = sum(len(s.get("titles", [])) for s in stats_list)
                        paragraphs.append([{"tag": "text", "text": f"共 {total_count} 条"}])
                    else:
                        paragraphs.append([{"tag": "text", "text": f"共 {total_groups} 组"}])
                    bullet = CONFIG.get("FEISHU_ITEM_BULLET", "•")
                    seen_keys = set()
                    for idx, s in enumerate(stats_list, 1):
                        ...
                        if idx == 1:
                            ...
                            for i, t in enumerate(slice_titles, 1):
                                ...
                                url = t.get("mobile_url") or t.get("url") or ""
                                segs = []
                                # 仅在关键词行使用图钉：这里显示关键词行已包含图钉，因此条目行不再重复图钉
                                # 若是新增，用新图标提示
                                prefix = f"{i}. [{src}] "
                                segs.append({"tag": "text", "text": prefix})
                                if url:
                                    segs.append({"tag": "a", "text": title_txt, "href": url})
                                else:
                                    segs.append({"tag": "text", "text": title_txt})
                                trail = ""
                                if time_info:
                                    trail += f" {time_info}"
                                trail += f" ({count_info}次) "
                                if rank_disp:
                                    trail += f"{rank_disp}"
                                segs.append({"tag": "text", "text": trail})
                                ...
                                if key:
                                    seen_keys.add(key)
                    if sep:
                        paragraphs.append([{"tag": "text", "text": sep}])
            else:
                stats_list = report_data.get("stats", [])
                valid_stats = [s for s in stats_list if s.get("count", 0) > 0]
                total_groups = len(valid_stats)
            ...
            if not is_combined:
                top_n = CONFIG.get("FEISHU_TOP_N_PER_GROUP", 5)
                bullet = CONFIG.get("FEISHU_ITEM_BULLET", "•")
                for idx, stat in enumerate(valid_stats, 1):
                    group_line = f"{bullet} [{idx}/{total_groups}] {stat.get('word','')}：{stat.get('count',0)} 条"
                    paragraphs.append([{"tag": "text", "text": group_line}])
                    ...
                    for i, t in enumerate(slice_titles, 1):
                        ...
                        url = t.get("mobile_url") or t.get("url") or ""
                        segs = []
                        segs.append({"tag": "text", "text": f"{i}. [{src}] "})
                        if url:
                            segs.append({"tag": "a", "text": title_txt, "href": url})
                        else:
                            segs.append({"tag": "text", "text": title_txt})
            ...
            for _ in range(2):
                paragraphs.append([{"tag": "text", "text": " "}])
            paragraphs.append([{"tag": "text", "text": f"更新时间：{now}"}])
            if link:
                paragraphs.append([{"tag": "a", "text": "打开完整报告", "href": link}])

            title_header = (
                "当日热点分析新闻报告" if is_combined else ("当日热点分析报告" if ("当日" in report_type) else f"TrendRadar 热点分析报告 - {report_type}")
            )
            payload = {
                "msg_type": "post",
                "content": {
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 检查飞书的响应状态
                if result.get("StatusCode") == 0 or result.get("code") == 0 or result.get("data"):
...
class NewsAnalyzer:
    ...
    def _send_notification_if_needed(
        ...
        if (
            ...
            send_to_notifications(
                ...
                new_titles,
                id_to_name,
                update_info if update_info is not None else self.update_info,
                self.proxy_url,
                mode=mode,
    ...
    def _generate_summary_report(self, mode_strategy: Dict) -> Optional[str]:
        ...
        docs_dir = CONFIG.get("GITHUB_PAGES_DOCS_DIR", "").strip()
        if docs_dir:
            try:
                date_str = get_beijing_time().strftime("%Y-%m-%d")
                target_dir = Path(docs_dir) / "reports"
                target_dir.mkdir(parents=True, exist_ok=True)
                target_file = target_dir / f"{date_str}.html"
                shutil.copy2(html_file, target_file)
                files = sorted(target_dir.glob("*.html"), reverse=True)
                items = [f"<li><a href='reports/{p.name}'>{p.stem}</a></li>" for p in files]
                html = "<!DOCTYPE html><html lang='zh-CN'><head><meta charset='utf-8'/><title>TrendRadar 历史报告</title><meta name='viewport' content='width=device-width, initial-scale=1'/><style>body{font-family:system-ui,-apple-system,Segoe UI,Roboto;max-width:960px;margin:40px auto;padding:0 16px}h1{font-size:22px;margin-bottom:10px}ul{line-height:1.9}a{color:#0366d6;text-decoration:none}</style></head><body><h1>TrendRadar 历史报告</h1><ul>" + "".join(items) + "</ul></body></html>"
    ...
    def _execute_mode_strategy(
        ...
        if self.report_mode == "current":
            ...
            if analysis_data:
                ...
                summary_html = None
                if mode_strategy["should_send_realtime"] and not (CONFIG.get("COMBINE_CURRENT_DAILY", False) or CONFIG.get("COMBINE_CURRENT_INCREMENTAL", False)):
                    self._send_notification_if_needed(
                        stats,
                ...
                # 回退：使用当前抓取结果生成最小报告，避免整体流程中断
                title_info = self._prepare_current_title_info(results, time_info)
                fallback_word_groups, fallback_filter_words = load_frequency_words()
                stats, html_file = self._run_analysis_pipeline(
                    results or {},
                    self.report_mode,
                    title_info,
                    {},
                    fallback_word_groups,
        ...
        if mode_strategy["should_generate_summary"]:
            if mode_strategy["should_send_realtime"]:
                if CONFIG.get("COMBINE_CURRENT_DAILY", False):
                    analysis_data = self._load_analysis_data()
                    if analysis_data:
                        (
                            all_results2,
                        ...
                        stats_daily, summary_html = self._run_analysis_pipeline(
                            all_results2,
                            "daily",
                            title_info2,
                            new_titles2,
                        ...
                        self._send_notification_if_needed(
                            ...
                            "daily",
                            failed_ids=failed_ids,
                            new_titles=new_titles2,
                            id_to_name=id_to_name2,
                            html_file_path=summary_html,
                            update_info={
                                "combined_sections": [
                                    {
                                        "icon": "📈",
                ...
                elif CONFIG.get("COMBINE_CURRENT_INCREMENTAL", False):
                    analysis_data = self._load_analysis_data()
                    if analysis_data:
                        (
                            all_results2,
                        ...
                        # 增量统计：基于当次新增标题集合
                        inc_new_titles = detect_latest_new_titles([p["id"] for p in CONFIG["PLATFORMS"]])
                        stats_inc, _ = self._run_analysis_pipeline(
                            all_results2,
                            "incremental",
                            title_info2,
                            inc_new_titles,
                        ...
                        total_inc = sum(len(s.get("titles", [])) for s in stats_inc or [])
                        if total_inc > 0:
                            # 合并推送：当前榜单 + 增量
                            self._send_notification_if_needed(
                                stats_inc,
